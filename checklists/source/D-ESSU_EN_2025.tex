...
\begin{checklist}{Before Enginge Start}
  ...
  \item{Parking Break}{set}
    \hint{Apply pressuere and set, then 'pump'.}
...
\begin{checklist}{Continue: Enginge Start}
  ...
  \decision{Throttle}
    \step{Coolant Tmp. <0$^{\circ}$C Thrtl. >= 60\%}
    \step{Coolant Tmp. >0$^{\circ}$C Thrtl. ca. 45\%}
  \item{Starter}{Pressed}
    \hint{Hold "Start Power" and press "START"}
...
\begin{checklist}{Engine Run Up}
  ...
  \decision{Pump and Fuel Supply Check}
    ...
    \step{Pressure still within limits: CHECKED}
...
\begin{checklist}{Continue: Engine Run Up}
  \decision{Wastegate/PVC/Lane}
    \step{Confirm BRAKES are fully set.}
  ...
  \item{Propeller Control}{checked}
    \hint{Cycle 3 times. Move lever slow }
    \hint{backwards, fast forward.}
